...
const BillCard = ({ BillName, amount, createdAt, billId, fetchData }) => {
    ...
    const handleDeleteClick = async () => {
        const response = await fetch(`/api/bill?billId=${billId}&user=${localStorage.getItem("username")}`, {
            method: "DELETE",
        });
        ...
        <div className=" bg-base-100 w-full shadow-xl rounded-md">
            <div className="flex h-full p-5 flex-col justify-between gap-4">
                ...
                <div className="pt-5 flex gap-3">
                    <button className="btn flex-1 hover:bg-green-600 hover:text-white bg-white text-green-600 border-green-600" onClick={handleSplitClick}>
                        Split
                    </button>
                    <button className="btn flex-1 hover:bg-red-500 hover:text-white border-red-500 text-red-500 bg-white" onClick={handleDeleteClick}>
                        Delete
                    </button>
                </div>
            </div>
